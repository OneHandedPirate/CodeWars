...
# variable warning for morse_codes because it's no longer used and kept only for
# old solutions.
...
def decode_morse(s: str) -> str:
    return ' '.join(''.join(MORSE_CODE[letter] for letter in word.split(' ')) for word in s.strip().split('   '))
